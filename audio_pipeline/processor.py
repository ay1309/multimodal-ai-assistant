...
def encode_audio(audio_bytes: bytes, media_type: str) -> dict:
    if media_type not in SUPPORTED_AUDIO_TYPES:
        raise ValueError(f"Not supported audio type: {media_type}")
...
def build_audio_message(audio_bytes: bytes, media_type: str, prompt: str = "Transcribe this audio and respond to the content.") -> dict:
    return {
        "role": "user",
        "content": [
